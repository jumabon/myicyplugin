...
public class SODAStorm3D extends Plugin implements Block, PluginBundled
{
    ...
    VarDoubleArrayNative detections1_z = new VarDoubleArrayNative("Detections 1 (z)", null);
    ...
    VarDoubleArrayNative detections2_z = new VarDoubleArrayNative("Detections 2 (z)", null);
    ...
    VarDoubleArrayNative detections1C_z = new VarDoubleArrayNative("Coupled Detections 1 (z)", null);
    ...
    VarDoubleArrayNative detections2C_z = new VarDoubleArrayNative("Coupled Detections 2 (z)", null);
    ...
    VarDoubleArrayNative detections1S_z = new VarDoubleArrayNative("Single Detections 1 (z)", null);
    ...
    VarDoubleArrayNative detections2S_z = new VarDoubleArrayNative("Single Detections 2 (z)", null);
    ...
    public void declareInput(VarList inputMap)
    {
        ...
        inputMap.add("Detections 1 (z)", detections1_z);

        inputMap.add("Detections 2 (x)", detections2_x);
        inputMap.add("Detections 2 (y)", detections2_y);
        inputMap.add("Detections 2 (z)", detections2_z);
    ...
    public void declareOutput(VarList outputMap)
    {
        ...
        outputMap.add("Coupled Detections 1 (z)", detections1C_z);
        outputMap.add("Coupled Detections 2 (x)", detections2C_x);
        outputMap.add("Coupled Detections 2 (y)", detections2C_y);
        outputMap.add("Coupled Detections 2 (z)", detections2C_z);
        ...
        outputMap.add("Single Detections 1 (z)", detections1S_z);
        outputMap.add("Single Detections 2 (x)", detections2S_x);
        outputMap.add("Single Detections 2 (y)", detections2S_y);
        outputMap.add("Single Detections 2 (z)", detections2S_z);
    }
    ...
    private void performAnalysis() throws InterruptedException {
        ...
        {
            Point3D pt = new Point3D.Double(detections1_x.getValue()[i], detections1_y.getValue()[i],
                    detections1_z.getValue()[i]);
            detections1.add(pt);
        }
        for (int i = 0; i < detections2_x.getValue().length; i++)
        {
            Point3D pt = new Point3D.Double(detections2_x.getValue()[i], detections2_y.getValue()[i],
                    detections2_z.getValue()[i]);
            detections2.add(pt);
        }
        ...
        /*
         * for (Point3D pt:detection2_0){
         * for (Point3D pt2:detection2_0){
         * if (pt==pt2){}
         * else
         * {double dist = Math.sqrt(Math.pow(pt.getX()-pt2.getX(), 2)+Math.pow(pt.getY()-pt2.getY(), 2)+Math.pow(pt.getZ()-pt2.getZ(), 2));
         * if (dist<sparse_dist.getValue())
         * {detection2.remove(pt2);}
         * }}}}
         */

        double volume = ROIUtil.getUnion(list_roi).getNumberOfPoints() * ratio_zx;
        if (ROIUtil.getUnion(list_roi).getBounds5D().isInfiniteZ())
        {
        ...
        Window3D[][][] imageWindows_1 = new Window3D[(int) (sequence.getWidth()
                / window_size)][(int) (sequence.getHeight() / window_size)][(int) (sequence.getSizeZ() / max_z)];
        imageWindows_1 = Window3D.window_tab(detection, sequence, maxdist, max_z);
        Window3D[][][] imageWindows_2 = new Window3D[(int) (sequence.getWidth()
                / window_size)][(int) (sequence.getHeight() / window_size)][(int) (sequence.getSizeZ() / max_z)];
        imageWindows_2 = Window3D.window_tab(detection2, sequence, maxdist, max_z);
        ...
        if (manual.getValue())
        {
            ...
            SODAblock.manual_computation3D(detection, detections2, distance_fit, col_ev, ratio_zx);
        ...
        else
        {
            ...
            // double[][] Ktemp_old = Ripley3D.correlation(sequence.getSizeZ(),roi_t,detection,detection2, volume, nbdeta, nbdetb, N_fit,
            // distance_fit,ratio_zx);
            ////////////////////////////////////////////////////////
            Ktemp = Ripley3D.correlation_new(list_roi, imageWindows_1, imageWindows_2, volume, N_fit, distance_fit,
                    input_sequence.getValue(), nbdeta, nbdetb, ratio_zx);
            ...
            non_parametric_object.main3D_corr(N_fit, sequence, list_roi, detection, detection2, imageWindows_1,
                    imageWindows_2, distance_fit, N_h, ratio_zx, volume, results, delta_K, Ktemp, proba_dist, maximum,
                    pvalue);
        ...
        double[] c_1_z = new double[liste_app_detect.size()];
        double[] c_2_x = new double[liste_app_detect.size()];
        double[] c_2_y = new double[liste_app_detect.size()];
        double[] c_2_z = new double[liste_app_detect.size()];
        ...
        for (apparatedLocalizations as : liste_app_detect)
        {
            ...
            c_1_z[ind] = as.p1.getZ();
            c_2_x[ind] = as.p2.getX();
            c_2_y[ind] = as.p2.getY();
            c_2_z[ind] = as.p2.getZ();
        ...
        detections1C_z.setValue(c_1_z);
        detections2C_x.setValue(c_2_x);
        detections2C_y.setValue(c_2_y);
        detections2C_z.setValue(c_2_z);
        ...
        double[] s_1_z = new double[apparatedLocalizations.single1.size()];
        double[] s_2_x = new double[apparatedLocalizations.single2.size()];
        double[] s_2_y = new double[apparatedLocalizations.single2.size()];
        double[] s_2_z = new double[apparatedLocalizations.single2.size()];
        ...
        for (int i = 0; i < apparatedLocalizations.single1.size(); i++)
        {
            ...
            s_1_z[i] = apparatedLocalizations.single1.get(i).getZ();
        }
    ...
    private ArrayList<Point3D> detectionsInRoi(ArrayList<Point3D> detections, ArrayList<ROI> roi) throws InterruptedException {
        ...
        {
            if (union.contains(pt.getX(), pt.getY(), pt.getZ(), union.getPosition5D().getT(),
